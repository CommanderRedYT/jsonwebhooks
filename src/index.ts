...
const doFormatStrings = (
  str: string,
  replacements: Record<string, string>,
): string => str.replace(/\{\{(\w+)}}/g, (_, key) => replacements[key] || '');
...
const handleQuery = async (query: Query): Promise<void> => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Error fetching query ${query.name}: ${response.statusText}`,
      );
      return;
    }
    ...
    if (!(query.name in lastCondition)) {
      ...
      console.log(
        `Condition for query ${query.name} has not changed. Skipping webhook.`,
      );
      return;
    } else {
      console.log(
        `Condition for query ${query.name} has changed from ${lastCondition[query.name]} to ${condition}.`,
      );
      lastCondition[query.name] = condition;
    }
    ...
    if (!body && method === 'POST') {
      console.warn(
        `No body defined for query ${query.name} when condition ${condition ? 'occurs' : 'does not occur'}.`,
      );
    }

    const parsedCommonBody = query.commonBody
      ? JSON.parse(query.commonBody)
      : {};
    const bodyWhenOccurs = query.bodyWhenOccurs
      ? JSON.parse(query.bodyWhenOccurs)
      : {};
    const bodyWhenNotOccurs = query.bodyWhenNotOccurs
      ? JSON.parse(query.bodyWhenNotOccurs)
      : {};
    ...
    const webhookResponse = await fetch(query.webhookUrl, {
      ...
      body:
        method === 'POST'
          ? doFormatStrings(JSON.stringify(actualBody), {
              currentTime,
    ...
    if (!webhookResponse.ok) {
      console.error(
        `Error sending webhook for query ${query.name}: ${webhookResponse.statusText}`,
        {
    ...
    console.log(
      `Webhook sent successfully for query ${query.name}. Condition: ${condition ? 'met' : 'not met'}.`,
    );
  } catch (error) {
...
const main = async (args: Args): Promise<void> => {
  ...
  try {
    ...
    console.error(
      `Error reading or parsing configuration file: ${args.config}`,
    );
    console.error(error);
    process.exit(1);
  }
  ...
  const { queries } = config as Config;
  ...
  for (const actualQuery of queries) {
    ...
    if (
      typeof query.webhookMethod !== 'string' ||
      !['GET', 'POST'].includes(query.webhookMethod)
    ) {
      console.error(
        "Each query must have a valid 'webhookMethod' ('GET' or 'POST').",
      );
      process.exit(1);
    }
    ...
    // optional fields
    if (
      typeof query.queryHeaders !== 'undefined' &&
      typeof query.queryHeaders !== 'object'
    ) {
    ...
    if (
      typeof query.invert !== 'undefined' &&
      typeof query.invert !== 'boolean'
    ) {
    ...
    if (
      typeof query.resend !== 'undefined' &&
      typeof query.resend !== 'boolean'
    ) {
    ...
    if (
      typeof query.commonBody !== 'undefined' &&
      typeof query.commonBody !== 'string'
    ) {
    ...
    if (
      typeof query.bodyWhenOccurs !== 'undefined' &&
      typeof query.bodyWhenOccurs !== 'string'
    ) {
    ...
    if (
      typeof query.bodyWhenNotOccurs !== 'undefined' &&
      typeof query.bodyWhenNotOccurs !== 'string'
    ) {
